...
class FreelanceApplication(BaseModel):
    ...
    def validate_strict_qatar_id(cls, value: str) -> str:
        ...
        if not re.match(r"^[23]\d{10}$", clean_qid):
            raise ValueError("Invalid QID structure. Requires a valid 11-digit Qatar ID.")
        return clean_qid
...
async def send_whatsapp_message(to_number: str, message: str):
    ...
    if not clean_number.startswith("974") and len(clean_number) == 8:
        clean_number = f"974{clean_number}"
    formatted_to = f"whatsapp:+{clean_number}"
    payload = {"From": from_number, "To": formatted_to, "Body": message}
    async with httpx.AsyncClient() as client:
        try:
            await client.post(gateway_url, data=payload, auth=(account_sid, auth_token))
        except Exception:
            pass
...
async def create_job(job: JobSubmission, request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # NOTE: Ensure columns 'email' and 'problem_category' exist in your Supabase table schema
        supabase_payload = {
        ...
        async with httpx.AsyncClient() as client:
            ...
            if db_response.status_code == 400:
                raise HTTPException(
                    status_code=400, 
                    detail=f"Database Alignment Error: Please verify that columns 'email' and 'problem_category' exist inside your Supabase 'jobs' table. Response: {db_response.text}"
                )
            db_response.raise_for_status()
            inserted_records = db_response.json()
        
        formatted_response = map_to_api_contract(inserted_records[0])
        tracking_url = f"https://maynd-stomir.vercel.app/status.html?id={formatted_response.get('id')}"
        notification_msg = f"🛠️ *Maynd Stomir - Request Confirmed*\n\nHi {formatted_response['full_name']},\nYour order is verified. Track live here: {tracking_url}"
...
async def lookup_job_by_phone(phone_number: str):
    try:
        ...
        base_url = SUPABASE_URL.strip().split("/rest/v1")[0]
        # Uses PostgREST alternative matching to verify both raw 8-digit and 974 formats
