...
async def verify_manual_payment(job_id: str):
    try:
        ...
        if job.get("status") != "awaiting_verification":
            raise HTTPException(status_code=400, detail=f"Job cannot be verified from status '{job.get('status')}'")

        dispatch_technician_for_job(job, internal_job_id)

        return {"success": True, "message": "Payment verified, technician dispatch triggered"}
