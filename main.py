...
@app.get("/workers", response_model=List[TechnicianDashboardResponse])
async def get_all_technicians():
    try:
        ...
        workers_url = f"{base_url.rstrip('/')}/rest/v1/freelance_applications?order=created_at.desc"
        jobs_url = f"{base_url.rstrip('/')}/rest/v1/jobs"

        async with httpx.AsyncClient() as client:
            workers_req = client.get(workers_url, headers=headers)
        ...
        for job in jobs_data:
            tech_name = job.get("assigned_technician")
            if not tech_name:
                continue
                
            tech_name_clean = tech_name.strip().lower()
            if tech_name_clean not in technician_metrics:
                technician_metrics[tech_name_clean] = {"assigned": 0, "completed": 0}
                
            status = (job.get("status") or "").upper()
            if status == "ASSIGNED":
                technician_metrics[tech_name_clean]["assigned"] += 1
            elif status == "COMPLETED":
                technician_metrics[tech_name_clean]["completed"] += 1
        ...
        for worker in workers_data:
            ...
            response_payload.append({
                ...
                "assigned_jobs_count": metrics["assigned"],
                "completed_jobs_count": metrics["completed"]
            })
